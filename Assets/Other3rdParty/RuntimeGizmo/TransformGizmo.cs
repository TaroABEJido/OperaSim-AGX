...
namespace RuntimeGizmos
{
	...
	{
        //20250522_A-tech Add
        public int MousInputForObjSelect = 0;
        ...
        private GameObject ConfirmDaialogUI;
		...
		void Update()
		{
			...
			if (GlobalVariables.SetMoveType == 2)
			{
				if (targetRootsOrdered[0] != null && myCamera.enabled == true)
				{
					...
					//if (targetRootsOrdered[0].gameObject.name.Contains("ic120_") == true)
					//{
					//	//GlobalVariables.ic120Counter = GlobalVariables.ic120Counter - 1;

					//	//Destroy(targetRootsOrdered[0].gameObject);
					//	//GameObject objMassBody = GameObject.Find(targetRootsOrdered[0].gameObject.name + "_SoilMassBody");
					//	//if (objMassBody != null) Destroy(objMassBody);
					//	//GameObject objMassJoint = GameObject.Find(targetRootsOrdered[0].gameObject.name + "_SoilMassJoint");
					//	//if (objMassJoint != null) Destroy(objMassJoint);

					//}
					//else if (targetRootsOrdered[0].gameObject.name.Contains("Camera_") == true)
					//{
					//	//Destroy(targetRootsOrdered[0].gameObject);
		...
		void TransformSelected()
		{
			if(mainTargetRoot != null)
			{
                ...
                if (nearAxis != Axis.None && Input.GetMouseButtonDown(MousInputForObjSelect))
				{
		...
		IEnumerator TransformSelected(TransformType transType)
		{
            ...
            while (!Input.GetMouseButtonUp(MousInputForObjSelect))
			{
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						for(int i = 0; i < targetRootsOrdered.Count; i++)
						{
                            ...
                            //if (GameObject.Find(target.name + "/base_link/track_link") != null)
                            //{
                            //    GameObject.Find(target.name + "/base_link/track_link").transform.Translate(movement, Space.World);
                            //}
                        }
			...
			//if (GameObject.Find(targetRootsOrdered[0].name + "/base_link/track_link") != null)
			//{
		...
		void GetTarget()
		{
			...
			if (GlobalVariables.ActionMode < 0 || GlobalVariables.ActionMode > 1)
			{
                ClearTargets();
                return;
            }
            ...
            //20250522 A-tech
            //if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
            if (nearAxis == Axis.None && Input.GetMouseButtonDown(MousInputForObjSelect) && myCamera.enabled == true)
			{
                ...
                if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
                    ...
                    while (target.parent != null) {
                        ...
                        Debug.Log("ChildParent Object: " + target.gameObject.name);
                    }
					...
					else if (GlobalVariables.ActionMode == 1 && target.gameObject.name.Contains("Camera_") == true)
					{
						//RenderTexture cameraTexture = new RenderTexture(160, 90, 24);
						//target.Find("Camera").gameObject.GetComponent<Camera>().enabled = true;
						//target.Find("Camera").gameObject.GetComponent<Camera>().targetTexture = cameraTexture;
					...
					//if (GameObject.Find(target.gameObject.name + "/base_link") != null)
					//{
					//	target = GameObject.Find(target.gameObject.name + "/base_link/body_link").transform;
					//}

					//Debug.Log("saParent Object: " + target.gameObject.name);

					//if(GameObject.Find(target.gameObject.name + "/base_link/track_link") != null)
					//{
					//    GameObject.Find(target.gameObject.name + "/base_link/track_link").SetActive(false);
					//}

					//if (target.gameObject.name == "Terrain")
					//if (target.gameObject.name == "Terrain_merge")
					//{
					//  Debug.Log("Terrain_merge...");
					//  ClearTargets();
					//	return;
					//}
		...
		public void RemoveTarget(Transform target, bool addCommand = true)
		{
			if(target != null)
			{
				UnityEngine.Debug.Log("RemoveTarget: " + target);
